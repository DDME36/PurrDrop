      ...
      {showMenu && (
        <>
          ...
          <div className="peer-actions-menu animate-pop-in" onClick={e => e.stopPropagation()}>
            ...
            <button className="peer-action-btn folder" onClick={() => { (window as Window & { triggerFolderSelect?: (p: Peer) => void }).triggerFolderSelect?.(peer); setShowMenu(false); }}>
              <FolderIcon /> <span>โฟลเดอร์</span>
            </button>
            <button className="peer-action-btn text" onClick={() => { (window as Window & { triggerTextShare?: (t: string) => void }).triggerTextShare?.(''); setShowMenu(false); }}>
