  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('set-mode', ({ mode, roomCode, password }: { mode: 'public' | 'wifi' | 'private'; roomCode?: string; password?: string }) => {
      ...
      if (mode === 'private' && nextRoomCode) {
        ...
        const existingRoom = rooms.get(nextRoomCode);
      ...
      broadcastPeers();
      socket.broadcast.emit('peer-updated', toPublicPeer(peer));
    });
    ...
    socket.on('file-offer', ({ to, file, fileId }: { to: string; file: { name: string; size: number; type: string }; fileId: string }) => {
      ...
      if (file.size < 0) {
        socket.emit('file-error', { error: 'File size is invalid' });
        return;
      }
    ...
    socket.on('relay-start', ({ to, fileId, name, size, mimeType }: { to: string; fileId: string; name: string; size: number; mimeType: string }) => {
      ...
      activeRelays.set(fileId, { startTime: Date.now(), from: socket.id, to: targetSocketId, size });
    ...
    socket.on('relay-chunk', ({ to, fileId, chunk }: { to: string; fileId: string; chunk: ArrayBuffer }, ack?: (success: boolean) => void) => {
      ...
      // Batch ACK: Only request acknowledgment from receiver every 8 chunks
      // This reduces round-trip overhead significantly on limited CPU (Render Free Tier)
      if (chunkCounter % 8 === 0) {
        ...
        if (ack) ack(true); // Return instant success for intermediate chunks
      }
    });
    ...
    socket.on('relay-end', ({ to, fileId }: { to: string; fileId: string }) => {
      ...
      setTimeout(() => {
        const targetSocketId = resolveSocketId(to);
        if (targetSocketId) {
          io.to(targetSocketId).emit('relay-end', { fileId });
        }
      }, 100);
    });
